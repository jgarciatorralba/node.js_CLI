...
program
  ...
  .action(cli=>{
    ...
    function showReviews(cli){
      const url = 'https://api.themoviedb.org/3/movie/' + cli.id + '/reviews?api_key=' + process.env.API_KEY
      const ora = require('ora')
        ...
        req.on('end', () => {
          ...
          if(results.length>0){
            ...
            if(dataObj.page > dataObj.total_pages){
              console.log(chalk.white('\n--------------------------------------------'))
              console.log(chalk.white('Page: ' + dataObj.page + ' from: ' + dataObj.total_pages))
            }
            results.forEach(review => {
              console.log(chalk.white('Author: ') + chalk.blueBright(review.author))
    ...
    function showMovie(cli){
      const url = 'https://api.themoviedb.org/3/movie/' + cli.id + '?api_key=' + process.env.API_KEY
      const ora = require('ora')
      const spinner = ora('Fetching the movie data...\n')
      spinner.spinner = ('unicorn')
      spinner.color = ('yellow')
